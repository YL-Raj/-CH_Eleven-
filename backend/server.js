...
// ─── Startup migration — add columns if they don't exist yet ─────────────────
(async () => {
  try {
    await pool.query(`ALTER TABLE match_info ADD COLUMN IF NOT EXISTS innings_loaded INTEGER DEFAULT 0;`);
    await pool.query(`ALTER TABLE players    ADD COLUMN IF NOT EXISTS did_not_play  BOOLEAN DEFAULT FALSE;`);
    await pool.query(`ALTER TABLE match_info ADD COLUMN IF NOT EXISTS result        TEXT DEFAULT '';`);
    await pool.query(`ALTER TABLE match_info ADD COLUMN IF NOT EXISTS team_a_score  TEXT DEFAULT '';`);
    await pool.query(`ALTER TABLE match_info ADD COLUMN IF NOT EXISTS team_b_score  TEXT DEFAULT '';`);
  ...
  try {
    ...
    if (ext === "json") {
      ...
      for (const line of lines) {
        ...
        if (!cols[0]) continue;
        if (cols[0].toUpperCase() === "TEAM") { currentTeam = cols[1] || ""; teamCount[currentTeam] = 0; continue; }
        if (cols[0].toUpperCase() === "PLAYER" || cols[0].toLowerCase() === "name") continue; // header
        const name = cols[0]; const role = cols[1] || "BAT";
        if (!name || name.toLowerCase() === "name") continue;
        teamCount[currentTeam] = (teamCount[currentTeam] || 0);
        if (teamCount[currentTeam] < 15) {
          players.push({ name, cric_team: currentTeam, role: ["BAT","BOWL","AR","WK"].includes(role.toUpperCase()) ? role.toUpperCase() : "BAT" });
